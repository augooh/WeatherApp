package com.app.weatherapp.common.custom
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        if (childCount > 0) {
            ...
            if (root.childCount > 0) {
                ...
                val y = (dY + temperatureView.getXPoint()).toInt()
                mPath.reset()
                mPath.moveTo(x.toFloat(), y.toFloat())

                for (i in 0 until root.childCount - 1) {
                    val child: HourlyWeatherItem = root.getChildAt(i)
                    val child1: HourlyWeatherItem = root.getChildAt(i+1)
